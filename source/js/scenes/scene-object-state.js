...
import {convertDegreesToRadians, rotate, calculateDY1} from 'helpers/calculator';
...
export const renderChart = (context, [x1, x2, dx], fx, angle) => {
  context.beginPath();
  for (let x = x1; x <= x2; x += dx) {
    const y = fx(x);
    if (x === x1) {
      context.moveTo(...rotate([x, y], angle));
    } else {
      context.lineTo(...rotate([x, y], angle));
    }
  }
  context.stroke();
};
...
export const renderTangent = (context, [x1, x2, dx], fx, dfx) => {
  for (let x = x1; x <= x2; x += dx) {
    const y = fx(x);
    const x_ = x + dx / 2;
    const y_ = calculateDY1(x, y, x_, dfx(x));
...
export class SceneObjectState {
  ...
  doRenderImage(context, image) {
    context.drawImage(
      image,
      -this.centerX * this.width * Math.sign(this.scaleX),
      -this.centerY * this.height * Math.sign(this.scaleY),
      this.width * Math.sign(this.scaleX),
      this.height * Math.sign(this.scaleY),
    );
  }
}
