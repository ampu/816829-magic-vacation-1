...
import {createCalculator, createCompositeCalculator, createRangeSlowdown} from 'helpers/calculator';
import {FrameAnimation} from 'helpers/frame-animation';
import {easeInOut} from 'helpers/easings';

export default new SceneObject({
  images: [
    document.querySelector(`.result__image-ice img`),
    document.querySelector(`.result__image-animal img`),
  ],
  state: {
  ...
  onRenderObject(context, {ice, animal}, [iceImage, animalImage]) {
    iceImage.style = animalImage.style = [
      // `display: flex`,
      'filter: sepia(100%)',
    ].join(`; `);
